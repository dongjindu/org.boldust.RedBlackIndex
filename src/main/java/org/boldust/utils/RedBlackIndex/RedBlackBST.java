...
public class RedBlackBST<Key, Value, KC extends Comparator<Key>> {
    ...
    *************************************************************************/
    private boolean check() {
        ...
        if (!isBST())            logger.log(Level.ERROR, "Not in symmetric order");
        if (!isSizeConsistent()) logger.log(Level.ERROR, "Subtree counts not consistent");
        if (!isRankConsistent()) logger.log(Level.ERROR, "Ranks not consistent");
        if (!is23())             logger.log(Level.ERROR, "Not a 2-3 tree");
        if (!isBalanced())       logger.log(Level.ERROR, "Not balanced");
